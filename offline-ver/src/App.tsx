...
import { Shield, Zap, AlertTriangle, CheckCircle2, Cpu, Info } from 'lucide-react';

const SHAPE_TEMPLATES_OFFLINE = [
...
const MODULE_LIBRARY_OFFLINE = SHAPE_TEMPLATES_OFFLINE.map(s => s.split('').map(c => c === '1'));
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-200 font-sans selection:bg-[#00BCD4] selection:text-white overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="absolute top-4 left-4 z-50 text-[8px] font-mono opacity-20 hidden sm:block">
        &gt; DEPLOY_ID: OFFLINE_AUTONOMOUS<br/>
        &gt; NODE_READY: TRUE
      </div>
          ...
          >
            <div className="space-y-4 animate-float">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[#00BCD4] font-mono text-[10px] sm:text-sm tracking-[0.2em] sm:tracking-[0.4em] uppercase opacity-60">Версия системы 9.4.0 // OFFLINE_MODE</p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 justify-center items-center text-[8px] sm:text-[10px] font-mono tracking-widest text-slate-500 uppercase">
                ...
                <div className="flex items-center gap-2">
                  <Zap size={12} className="text-[#00BCD4]" /> КАНАЛ_СВЯЗИ: ЛОКАЛЬНЫЙ_АВТОНОМНЫЙ
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="space-y-2">
              <div className="flex justify-between items-end border-b border-white/5 pb-2">
                ...
                <span className="text-[10px] font-mono text-[#00BCD4]">LOCAL_MODE: ACTIVE</span>
              </div>
...
function ShapeHacking({ onComplete, onSyncChange }: { onComplete: (s: boolean) => void, onSyncChange: (v: number) => void }) {
  ...
  useEffect(() => {
    const solutionIdx = Math.floor(Math.random() * MODULE_LIBRARY_OFFLINE.length);
    const solutionModule = MODULE_LIBRARY_OFFLINE[solutionIdx];
    setTargetShape(solutionModule.map(v => !v));
    setCurrentIdx(Math.floor(Math.random() * MODULE_LIBRARY_OFFLINE.length));
    setAppliedModule(null);
    setIsChecking(false);
  }, [level]);

  const handleInject = React.useCallback(() => {
    if (MODULE_LIBRARY_OFFLINE.length === 0 || targetShape.length === 0 || isChecking) return;
    
    sounds.playClick();
    setIsChecking(true);
    const currentModule = MODULE_LIBRARY_OFFLINE[currentIdx];
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft') setCurrentIdx(prev => (prev - 1 + MODULE_LIBRARY_OFFLINE.length) % MODULE_LIBRARY_OFFLINE.length);
      if (e.key === 'ArrowRight') setCurrentIdx(prev => (prev + 1) % MODULE_LIBRARY_OFFLINE.length);
    ...
    >
      ...
      <div className="w-full flex flex-col gap-4 sm:gap-6 items-center">
        ...
        <div className="flex items-center gap-2 sm:gap-4 w-full px-1">
          <button 
            onClick={() => !isChecking && setCurrentIdx(prev => (prev - 1 + MODULE_LIBRARY_OFFLINE.length) % MODULE_LIBRARY_OFFLINE.length)} 
            disabled={isChecking}
          ...
          <div className="flex-1 relative flex items-center justify-center h-40 sm:h-48 overflow-hidden pointer-events-none">
             <AnimatePresence initial={false} mode="popLayout">
                {[-1, 0, 1].map((offset) => {
                  const idx = (currentIdx + offset + MODULE_LIBRARY_OFFLINE.length) % MODULE_LIBRARY_OFFLINE.length;
                  ...
                  return (
                    ...
                    >
                      {MODULE_LIBRARY_OFFLINE[idx].map((active, i) => (
                        <div
             ...
             <div className="absolute top-0 right-0 text-[7px] sm:text-[8px] font-mono opacity-40 text-[#00BCD4] px-2 py-0.5 sm:py-1 bg-slate-950 rounded-bl-lg border-l border-b border-white/5">
                {currentIdx + 1}/{MODULE_LIBRARY_OFFLINE.length}
             </div>
          </div>

          <button 
            onClick={() => !isChecking && setCurrentIdx(prev => (prev + 1) % MODULE_LIBRARY_OFFLINE.length)}
            disabled={isChecking}
            className={`w-8 h-8 sm:w-10 sm:h-10 shrink-0 bg-white/5 border border-white/10 rounded-full text-[#00BCD4] hover:bg-[#00BCD4] hover:text-black active:scale-90 transition-all flex items-center justify-center font-black z-20 ${isChecking ? 'opacity-10' : ''}`}
          >
            →
        ...
        <div className="flex gap-1 sm:gap-1.5 w-full justify-center">
          {MODULE_LIBRARY_OFFLINE.map((_, i) => (
            <div 
              key={`dot-${i}`} 
